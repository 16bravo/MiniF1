...
// Modules used: utils, weather, tires, physics, ranking, flags, pits, drs, animation, recording, simulation
...
// Initialize recorder with metadata
RaceRecorder.init({
    circuit: circuit,
...
// Flag to update circuit data after loading
...
function showChampionshipResultsButton() {
    if (localStorage.getItem('championshipActive') === 'true') {
        ...
        btn.onclick = () => {
            ...
            // Remplace le résultat à l'index de la course en cours
...
async function loadCircuitData(circuit) {
    ...
    if (circuitData) {
        ...
        // Update recorder metadata with circuit data
        if (!recorderMetadataSet) {
            // Re-initialize with full metadata
            RaceRecorder.init({
                circuit: circuit,
                selectedCircuit: selectedCircuit,
...
function frame(interval) {
    ...
    if (leader_total_length <= raceLength && raceTimeLeft > 0 && eventTimeLeft > 0) { //While the race is running
        ...
        for (let pos = 0; pos < sorted_indices.length; pos++) {
            ...
            if (
                ...
                // Expected movement
                let expected_length = (generateNormalRandom(baseSpeed*9 + drivers[i].level*10, 6)/30) * gripFactor * drivers[i].tirePerf * drivers[i].carPerf * flagFactor;
                ...
                // CORRECTED LOGIC - Three cases: preserve current driver's lap count, only adjust track position
                const frontDriver = drivers[frontIndicesRanking[i]];
                const currentLaps = Math.floor(drivers[i].totalLength / circuitLength);
                const currentCircuitPos = drivers[i].totalLength % circuitLength;
                const frontCircuitPos = frontDriver.totalLength % circuitLength;

                let target_totalLength;

                if (frontDriver.totalLength >= drivers[i].totalLength) {
                    // Case 1: Front driver ahead overall
                    target_totalLength = frontDriver.totalLength;
                } else if (frontCircuitPos > currentCircuitPos) {
                    // Case 2: Front driver is retardataire but ahead on track
                    target_totalLength = (currentLaps * circuitLength) + frontCircuitPos;
                } else {
                    // Case 3: Front driver is retardataire and behind on track
                    target_totalLength = ((currentLaps + 1) * circuitLength) + frontCircuitPos;
                }

                // Progressive approach to target (with distanceFactor)
                drivers[i].totalLength = distanceFactor * target_totalLength + (1 - distanceFactor) * total_driver_length_expected;
            } else
                // NORMAL CONDITIONS
            {
                // CRASH
                ...
                // Expected movement
                let expected_length = (generateNormalRandom(baseSpeed*9 + drivers[i].level*10, 6)/30) * gripFactor * drivers[i].tirePerf * drivers[i].carPerf;
                let expected_speed = (expected_length) * 180/4000; // Expected full speed
            ...
            if ( 
                ...
                ((leaderDistanceLeft < 100 && drivers[i].tireState < 0.33) || // ... or there is less than 100km left and the tire is quite worn
    ...
    if (currentFrame >= frames) {
        ...
        if (leader_total_length <= raceLength && raceTimeLeft > 0 && eventTimeLeft > 0) { // End of race: either the leader has finished or the race time has elapsed
            ...
            // ADD DOWNLOAD BUTTON
            const recording = RaceRecorder.stop();
            ...
            downloadBtn.onclick = () => RaceRecorder.downloadRecording(recording);
